...
{   //int rank=rowNum;
	...
	//The sort Matrix means they are in order of their leading one values. left most leading ones first
	sortMatrix(leadOne);
	//The target column is the column of the first leading 1 value that is not in a cleared column
	int target=leadOne[colReduced];
	/*
	 * Since the matrix is sorted by leading ones, a row above can either have a lead one to the left or same column
	 * If there is a leadone in the same column, it would be cleared when the row above
	 * Leading ones to the left of the leading one of a row below it is proper form for rowEchelonForm
	 * Therefor only rows below have to be cleared
	 */
	
	for (int i=colReduced+1;i+1<rowNum;i++)
	{
		//This checks if two lead ones are in
		if (target==leadOne[i])
		{
			rows[i]=rows[i].subtractRows(rows[target]);
			//This updates the lead row information
			leadOne[i]=rows[i].leadLocation();
			//If after subtraction the row is an empty row, the rank is reduced by one (a leading 1 location equal to column length means a row of all zeros
	...
	/*Now the column is reduced, repeat the process with the next column, 
	 * unless this is the last column, then Matrix is in reduced row echelon form, 
	* then it returns the rank.
	*/
	colReduced++;
...
{
	String originalMatrix=this.toString();
	...
	System.out.println("End of Original Matrix");
	System.out.println("RowReducedVersion");
	printMatrix();
	System.out.println("End of RowReducedVersion");
	}
	return (rank==rowNum);
}
public Matrix subtractMatrix(Matrix a)
{ 
	Matrix result=new Matrix(rowNum, rows[0].col);
	if (rowNum==a.rowNum && rows[0].col==a.rows[0].col)
	{
	
		for (int i=0; i<rowNum;i++)
		result.rows[i]=rows[i].subtractRows(a.rows[i]);
	}
	else
...
System.out.println("Matrix for which the eigen value is been tested");
a.printMatrix();
System.out.println("End of tested Matrix");
Matrix temp=new Matrix(test, a.rowNum);
System.out.println("Identity matrix multipled by the tested integer being tested as an eigenvalue");
temp.printMatrix();
System.out.println("End of identity Matrix");
Matrix result=a.subtractMatrix(temp);
System.out.println("Matrix being tested minus the Indentity Matrix");
result.printMatrix();
System.out.println("End of new Matrix.");
System.out.println("");
return !result.reduceRowEchelonMatrix();
}
public String toString()
{
	...
	for (int i=0; i<rowNum;i++)
	{
		writeMatrix.append(rows[i].toString()+"\n");
